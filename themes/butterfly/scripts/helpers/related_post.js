...
hexo.extend.helper.register('related_posts', function (currentPost, allPosts) {
  ...
  if (relatedPosts.length > 0) {
    ...
    for (let i = 0; i < Math.min(relatedPosts.length, limitNum); i++) {
      ...
      if (dateType === 'created') {
        result += `<div class="content is-center"><div class="date"><i class="iab carbon--calendar-heat-map fa-fw"></i> ${this.date(relatedPosts[i].created, hexoConfig.date_format)}</div>`
      } else {
        result += `<div class="content is-center"><div class="date"><i class="iab carbon--notification-new"></i> ${this.date(relatedPosts[i].updated, hexoConfig.date_format)}</div>`
      }
